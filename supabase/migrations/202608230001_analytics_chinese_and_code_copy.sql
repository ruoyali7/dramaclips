...
returns jsonb language sql security definer set search_path = public as $$
  with scoped as (select name, session_id, drama_slug, source from public.tracking_events where occurred_at >= since_at),
  ...
  select jsonb_build_object(
    'visits', (select count(*)::int from visits), 'sessions', (select count(distinct session_id)::int from visits),
    ...
    'redirects', (select count(*)::int from scoped where name = 'redirect_success'), 'events', (select count(*)::int from scoped),
