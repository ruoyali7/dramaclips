...
create index if not exists drama_cover_posts_drama_idx
  on public.drama_cover_posts(drama_slug, updated_at desc);

alter table public.drama_cover_posts enable row level security;
revoke all on table public.drama_cover_posts from anon, authenticated;
grant select, insert, update on table public.drama_cover_posts to service_role;
