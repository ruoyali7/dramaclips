...
begin
  ...
  where yixiaoer_action is not null
    and (status in ('validating','publishing') or (status='scheduled' and scheduled_at is not null and scheduled_at <= now()))
  ...
  update public.publish_packages set
    status=case when picked.status='scheduled' then 'publishing' else picked.status end,
