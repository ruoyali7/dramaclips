...
export function VizardStudio({ sources }: { sources: Source[] }) {
  ...
  </form><section className="vizard-projects"><span>03 · Vizard projects & edit info</span>{projects.length ? projects.map((project) => <form className="vizard-project" key={project.id} onSubmit={(event) => void saveProject(event, project)}><div><b>{project.projectName}</b><small>Project {project.vizardProjectId} · EP {project.episodeNumber} · {project.status}</small></div><label>Final video URL<input name="finalVideoUrl" type="url" defaultValue={project.finalVideoUrl || ""} placeholder="https://.../final.mp4" required /></label><label>Asset label<input name="finalLabel" defaultValue={project.finalLabel || `${source?.title || "Drama"} · Vizard · EP ${project.episodeNumber}`} /></label><div className="vizard-edit-grid"><label>Template / Brand Kit<input name="template" placeholder="Template ID or name" /></label><label>Subtitle style<input name="subtitleStyle" placeholder="Position, font, color" /></label><label>Selected ranges<input name="selectedRanges" placeholder="00:04–00:28" /></label></div><label>Edit notes<textarea name="notes" placeholder="标题、字幕、镜头调整和人工修改说明" /></label><button disabled={projectBusy === project.id}>{projectBusy === project.id ? "Saving…" : project.finalVideoUrl ? "Update asset info" : "Save final asset to Publish Center"}</button></form>) : <p>No Vizard projects for this drama yet.</p>}</section></>;
}
