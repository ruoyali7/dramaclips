...
export function VizardStudio({ sources }: { sources: Source[] }) {
  ...
  </form><section className="vizard-projects"><div className="vizard-section-heading"><div><span>03 · Vizard projects</span><h2>External edits</h2></div><small>Track the project here; completed clips arrive automatically through Webhook.</small></div>{projects.length ? <div className="vizard-project-list">{projects.map((project) => <form className="vizard-project" key={project.id} onSubmit={(event) => void saveProject(event, project)}><header><div><b>{project.projectName}</b><small>EP {project.episodeNumber} · Project {project.vizardProjectId}</small></div><i className={`vizard-status ${project.status}`}>{project.status === "submitted" ? "Processing" : project.status === "ready" ? "Ready" : project.status}</i></header>{project.finalVideoUrl ? <video className="vizard-final-preview" src={project.finalVideoUrl} controls preload="metadata" playsInline /> : <div className="vizard-waiting"><b>Waiting for Vizard</b><span>When processing finishes, Webhook will import the clips to R2 and Publish Center.</span></div>}<details><summary>Edit details</summary><div className="vizard-edit-grid"><label>Template / Brand Kit<input name="template" placeholder="Template ID or name" /></label><label>Subtitle style<input name="subtitleStyle" placeholder="Position, font, color" /></label><label>Selected ranges<input name="selectedRanges" placeholder="00:04–00:28" /></label></div><label className="vizard-notes">Notes<textarea name="notes" placeholder="Title, subtitles, shot changes, and manual edits" /></label></details><div className="vizard-project-footer"><label>Final video URL<input name="finalVideoUrl" type="url" defaultValue={project.finalVideoUrl || ""} placeholder="Optional manual fallback" /></label><label>Asset label<input name="finalLabel" defaultValue={project.finalLabel || `${source?.title || "Drama"} · Vizard · EP ${project.episodeNumber}`} /></label><button disabled={projectBusy === project.id}>{projectBusy === project.id ? "Saving…" : project.finalVideoUrl ? "Update details" : "Save manual fallback"}</button></div></form>)}</div> : <div className="vizard-empty"><b>No Vizard projects yet</b><span>Submit an episode above to start an external edit.</span></div>}</section></>;
}
