...
export function EpisodePlayer({drama,episodes,goHref,contentPromotionHref}:{drama:Drama;episodes:Episode[];goHref:string;contentPromotionHref?:string}) {
  ...
  return <div className="watch-stage"><div className="vertical-player">
    ...
    <div className="video-progress" role="slider" aria-label="Video progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(progress)} onPointerDown={event=>{event.currentTarget.setPointerCapture(event.pointerId);seek(event)}} onPointerMove={event=>{if(event.currentTarget.hasPointerCapture(event.pointerId))seek(event)}}><i style={{width:`${progress}%`}}/></div>
    {ended&&<div className="unlock-overlay"><LockKeyhole/><h3>Ready for the full story?</h3><p>Continue all remaining episodes in the official app.</p><a href={contentPromotionHref||goHref} onClick={event=>{track("watch_full_click",{dramaId:drama.id,dramaSlug:drama.slug,position:"episode_end"});void openFull(event,"episode_end")}}>Watch full series <ExternalLink/></a></div>}
  ...
  <aside className="watch-info"><span className="watch-kicker">#{drama.publicCode} · {drama.tags.join(" · ")}</span><h1>{drama.title}</h1><p>{drama.hook}</p><a className="watch-full" href={contentPromotionHref||goHref} onClick={event=>{track("watch_full_click",{dramaId:drama.id,dramaSlug:drama.slug,position:"below_player"});void openFull(event,"below_player")}}><Play fill="currentColor"/> Watch full series <ExternalLink/></a><small>{contentPromotionHref?"Your Content Code is copied automatically. Open ReelShort to watch this drama.":"Continue on ReelShort, DramaBox, or the available official app."}</small>{drama.promoCode&&<button className="rs-code" onClick={copy}><div><span>ReelShort search code</span><strong>{drama.promoCode}</strong></div>{copied?<Check/>:<Copy/>}</button>}<details><summary>About this drama</summary><p>{drama.description}</p></details></aside>
  </div>;
}
