...
export function EpisodePlayer({drama,episodes,goHref}:{drama:Drama;episodes:Episode[];goHref:string}) {
  ...
  return <div className="watch-stage"><div className="vertical-player">
    ...
    {ended&&<div className="unlock-overlay"><LockKeyhole/><h3>Ready for the full story?</h3><p>Continue all remaining episodes in the official app.</p><Link href={goHref} onClick={()=>track("watch_full_click",{dramaId:drama.id,dramaSlug:drama.slug,position:"episode_end"})}>Watch full series <ExternalLink/></Link></div>}
