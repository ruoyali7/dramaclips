...
function track(name:string, data:Record<string,unknown>) {
  const body=JSON.stringify({name,schemaVersion:1,occurredAt:new Date().toISOString(),...data});
...
export function EpisodePlayer({drama,episodes,goHref}:{drama:Drama;episodes:Episode[];goHref:string}) {
  ...
  const play=useCallback(()=>{video.current?.play();setPlaying(true);track("episode_start",{dramaId:drama.id,episodeId:episode.id,episodeNumber:episode.episodeNumber});},[drama.id,episode]);
  ...
  function select(next:number){if(next<0||next>=episodes.length)return;setIndex(next);setPlaying(false);setTimeout(()=>video.current?.play().then(()=>setPlaying(true)).catch(()=>{}),80);track("next_episode",{dramaId:drama.id,fromEpisode:episode.episodeNumber,toEpisode:next+1});}
  function onTime(){const el=video.current;if(!el||!el.duration)return;setProgress((el.currentTime/el.duration)*100)}
  function onEnded(){setPlaying(false);track("episode_complete",{dramaId:drama.id,episodeId:episode.id,episodeNumber:episode.episodeNumber});if(index<episodes.length-1)select(index+1);else setEnded(true)}
  async function copy(){if(!drama.promoCode)return;await navigator.clipboard.writeText(drama.promoCode);setCopied(true);track("promo_code_copy",{dramaId:drama.id});setTimeout(()=>setCopied(false),1800)}
  return <div className="watch-stage"><div className="vertical-player">
    ...
    {ended&&<div className="unlock-overlay"><LockKeyhole/><h3>Ready for the full story?</h3><p>Continue all remaining episodes in the official app.</p><Link href={goHref} onClick={()=>track("watch_full_click",{dramaId:drama.id,position:"episode_end"})}>Watch full series <ExternalLink/></Link></div>}
