...
def render(asset,candidate,target,drama_cover,content_code,cover_duration=.1):
 ...
 overlay=f"{base},drawtext=fontfile={filter_path(FONT)}:textfile={filter_path(code_text)}:reload=0:fontcolor=white@0.96:fontsize=30:x=38:y=38:box=1:boxcolor=black@0.52:boxborderw=11,{title_filters}"
 delay_ms=round(cover_duration*1000)
 graph=f"[0:v]{overlay},trim=duration={cover_duration},setpts=PTS-STARTPTS[cv];[1:v]{overlay},trim=duration={duration},setpts=PTS-STARTPTS[bodyv];[2:v]{overlay},trim=duration={ENDING_HOLD_SECONDS},setpts=PTS-STARTPTS[endv];[cv][bodyv][endv]concat=n=3:v=1:a=0[v];[1:a]asetpts=PTS-STARTPTS,afade=t=out:st={fade_start}:d=0.25,adelay={delay_ms}:all=1,apad,atrim=duration={total}[a]"
 ...
 qa={"frameZeroExtracted":frame0.exists() and frame0.stat().st_size>5000,"endingFrameExtracted":ending_frame.exists() and ending_frame.stat().st_size>5000,"dramaCoverApplied":drama_cover.exists(),"codeOnlyLabelApplied":bool(content_code),"endingHoldApplied":ENDING_HOLD_SECONDS>0,"durationWithinTolerance":abs(float(info["format"]["duration"])-total)<.2,"durationAtMost90Seconds":float(info["format"]["duration"])<=90.05,"streamsAligned":abs(float(video.get("duration",total))-float(audio.get("duration",total)))<.2,"portrait1080x1920":video.get("width")==1080 and video.get("height")==1920,"cleanEndingFrame":clean_end<=source_duration,"audioFadeApplied":bool(audio)}
