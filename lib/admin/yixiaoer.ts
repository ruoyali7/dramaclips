...
export function plainYixiaoerDescription(value:string){return value.replace(/<[^>]+>/g," ").replace(/(^|\s)#[A-Za-z0-9_-]+/g,"$1").replace(/\n{3,}/g,"\n\n").replace(/[ \t]{2,}/g," ").trim()}
export function buildYixiaoerPayload(platform:PublishingPlatform,accountId:string,video:Record<string,unknown>,pack:PublishPack,title:string){const name=yixiaoerPlatformName(platform);const description=plainYixiaoerDescription(pack.caption);const content:Record<string,unknown>={formType:"task"};if(platform==="youtube")Object.assign(content,{title:clip(title,100),description:clip(description,5000),tags:["Shorts","DramaClips","ShortDrama"],category:"22",license:"youtube",embeddable:true,madeForKids:false,visible:"public",containsSyntheticMedia:false,fps:10});if(platform==="tiktok")Object.assign(content,{description:clip(description,2200),visible:"public",comment:true,stitch:true,duet:true,aigc:false,business:false,yourOwn:false,collaborative:false,fps:10,isAdVideo:false});if(platform==="facebook")Object.assign(content,{title:clip(title,128),description:clip(description,2048)});if(platform==="instagram")Object.assign(content,{description:clip(description,2200),share_to_feed:true});return{action:"publish",publishType:"video",platforms:[name],publishChannel:process.env.YIXIAOER_PUBLISH_CHANNEL||"cloud",desc:title,publishArgs:{video,accountForms:[{platformAccountId:accountId,platformName:name,video,contentPublishForm:content}]}}}
