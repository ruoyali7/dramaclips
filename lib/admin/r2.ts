...
export async function listExpiredHookDrafts(before:Date,protectedKeys=new Set<string>()){let token="";const items:{key:string;bytes:number;modifiedAt:string}[]=[];do{const query:Record<string,string>={"list-type":"2",prefix:"dramas/"};if(token)query["continuation-token"]=token;const response=await signedR2Request("GET","",query);if(!response.ok)throw new Error(`R2 list returned ${response.status}`);const xml=await response.text();for(const match of Array.from(xml.matchAll(/<Contents>([\s\S]*?)<\/Contents>/g))){const block=match[1],key=xmlValue(block,"Key"),modifiedAt=xmlValue(block,"LastModified"),modified=new Date(modifiedAt);if(key.includes("/social/drafts/")&&modified<before&&!protectedKeys.has(key))items.push({key,bytes:Number(xmlValue(block,"Size")||0),modifiedAt})}token=xmlValue(xml,"NextContinuationToken")}while(token);return items}
export async function deleteExpiredHookDrafts(keys:string[]){for(const key of keys){if(!key.startsWith("dramas/")||!key.includes("/social/drafts/"))throw new Error("Cleanup may only delete hook drafts");const removed=await signedR2Request("DELETE",key);if(!removed.ok&&removed.status!==404)throw new Error(`R2 delete returned ${removed.status}`)}}
