...
export async function listVizardSources(){
  ...
  return rows.map(row=>({id:row.id,title:row.title,slug:row.slug,language:row.language,description:row.description,coverUrl:row.coverUrl,episodes:row.episodes.map(episode=>({episodeNumber:episode.episodeNumber,videoUrl:episode.videoUrl}))}));
}
