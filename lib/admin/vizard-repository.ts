...
export async function enqueueVizardSubmissions(input: VizardSubmissionInput[]) {
  ...
  await Promise.all(rows.map((row) => request(`vizard_submission_jobs?drama_id=eq.${encodeURIComponent(row.drama_id)}&episode_number=eq.${row.episode_number}&status=in.(canceled,failed)`, { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ ...row, status: "queued", attempt_count: 0, error_message: null, vizard_project_id: null, lease_owner: null, lease_expires_at: null, updated_at: new Date().toISOString() }) })));
