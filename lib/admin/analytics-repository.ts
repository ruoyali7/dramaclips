...
export type AnalyticsSummary={visits:number;sessions:number;previewStarts:number;previewCompletions:number;watchFullClicks:number;promoCodeCopies:number;rsRedirects:number;redirects:number;events:number;bySource:[string,number][];byDrama:[string,number][]};
