...
export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata> { const {slug}=await params; const d=dramas.find(x=>x.slug===slug); return d ? {title:d.title,description:d.hook,alternates:{canonical:`/d/${d.slug}`},openGraph:{images:[d.coverUrl]}} : {}; }
export default async function DramaDetail({params,searchParams}:{params:Promise<{slug:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}) { const {slug}=await params; const resolvedSearch=await searchParams; const d=dramas.find(x=>x.slug===slug && x.status==="published"); if(!d) notFound(); const query=trackingQuery(resolvedSearch); return <main className="detail-page"><RememberDrama drama={d}/><header className="detail-nav"><Link href="/"><ArrowLeft/></Link><Brand/><button aria-label="Save"><Bookmark/></button></header>
  ...
  <section className="related"><span className="kicker">You may also like</span><div className="card-grid">{dramas.filter(x=>x.id!==d.id).slice(0,3).map(x=><DramaCard key={x.id} drama={x}/>)}</div></section></main>; }
