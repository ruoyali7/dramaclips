...
export async function POST(request: NextRequest) {
  try { const { jobs } = schema.parse(await request.json()); const queued = await enqueueVizardSubmissions(jobs); const workerTrigger = await triggerRailwayWorker("vizard"); return NextResponse.json({ jobs: queued, status:"queued", workerTrigger }, { status:202 }); }
