  ...
  return <AdminShell active="Analytics">
    ...
    {summary ? <>
      <div className="stats">
        ...
        <Stat label="Watch Full clicks" value={summary.watchFullClicks.toLocaleString()} delta="Continue watching clicks · 继续观看点击" />
        <Stat label="Code copies" value={summary.promoCodeCopies.toLocaleString()} delta="Content Code copies · Content Code 复制" />
        <Stat label="Successful redirects" value={summary.redirects.toLocaleString()} delta="Reached RS link only · 仅代表到达 RS 链接" />
      </div>
      <section className="panel placeholder-panel"><span>Important · 重要说明</span><h2>Revenue is not live in this page</h2><p>This page tracks visits, playback, code copies, and redirects. RS orders and earnings must be imported separately. A successful redirect is not an install or a paid conversion. · 本页统计访问、播放、代码复制和跳转；RS 订单与收入需要单独导入，成功跳转不等于安装或付费。</p></section>
