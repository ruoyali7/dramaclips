...
export default async function Home({searchParams}:{searchParams:Promise<Record<string,string|string[]|undefined>>}) {
  ...
  return <main className="dc-home">
    ...
    <section className="dc-trending" id="all-dramas"><div className="dc-section-head"><div><span>Browse the library</span><h2>All dramas</h2><p>Pick a story and watch the available free chapters here.</p></div><Link href="#find-by-code">Find by code <ArrowRight/></Link></div><div className="dc-poster-grid">{dramas.map((drama)=><Link href={`/watch/${drama.slug}`} key={drama.id} className="dc-poster"><div><Image src={drama.coverUrl} alt="" fill sizes="(max-width:700px) 42vw, 220px"/><i><Play fill="currentColor"/></i><b>{episodes.filter(item=>item.dramaId===drama.id).length} free chapters</b></div><small>Content Code · {drama.promoCode || drama.publicCode} · {drama.tags[0]}</small><strong>{drama.title}</strong></Link>)}</div></section>
