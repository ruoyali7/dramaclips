...
统一出片规范由确定性渲染层负责，而不是交给 LLM：全片左上角只显示数据库中的 Content Referral Code 数字；开头 0.1 秒使用该剧 `coverUrl`；结尾保留完整对白后冻结最后一帧 1.8 秒，并加入低饱和、轻压暗、暗角以及克制的低频/空气感尾音。结尾占用也计入 90 秒硬上限。
