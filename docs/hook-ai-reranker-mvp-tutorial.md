...
规则候选窗口会在完整场景边界上提供多个自然长度版本，不设置产品层的最短时长，唯一硬上限是 90 秒（包括最终视频的封面帧）。真实测试中，旧实现只留下了 87–90 秒候选，让 reranker 没有长度选择；现在同一精彩点的短版和长版可以同时进入候选池，由 AI 根据剧情完整性自动选择，而不是追求某个固定秒数。

统一出片规范由确定性渲染层负责，而不是交给 LLM：全片左上角只显示数据库中的 Content Referral Code 数字；开头 0.1 秒使用该剧 `coverUrl`；结尾保留完整对白后冻结最后一帧约 2.1 秒，依次加入原画停顿、洋红光泄漏、白蓝闪光、暖橙红回落，并配合低频冲击、宽频 whoosh 和闪光噪声。结尾占用也计入 90 秒硬上限。
